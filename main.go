...
const (
	twimgHostName = "video.twimg.com"
	twimgTestURI  = "https://video.twimg.com/ext_tw_video/1251066844548489216/pu/vid/1280x720/qFITDS_AeTMnKW49.mp4"
...
func patch(args *args) {
	...
	if !args.Install || !args.Uninstall {
		println("Twvideo 패치를 확인중입니다")
	}

	lines, patched := readAllHosts(fs)

	if args.Install && patched {
		println("이미 Twvideo dns 패치가 되어있습니다.")
		return
	}
	if args.Uninstall && !patched {
		println("Twvideo dns 패치가 되어있지 않습니다.")
		return
	}
	...
	if patched {
		println("Twvideo cdn 패치를 제거하였습니다.")
	} else {
